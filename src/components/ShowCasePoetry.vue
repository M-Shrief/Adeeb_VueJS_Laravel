  ...
  <section id="chosen-verses">
    ...
    <div :class="grid">
      <div v-for="chosenVerse in chosenVerses" :key="chosenVerse.id"
        class="poetry-item">
        <div v-for="verse in chosenVerse.verse" :key="chosenVerse.id"
          :class="chosenVerse.verse.length == 1 ? 'one-verse' : 'two-verse'">
          <p class="first">{{ verse.first }}</p>
          <p class="sec" dir="ltr">{{ verse.sec }}</p>
        </div>
        <router-link :to="'/poem/' + chosenVerse.poem_id"
          class="details">{{ route.name == "poet" ? 'القصيدة الكاملة' :
            chosenVerse.poet.name + ' - القصيدة الكاملة' }}</router-link>
        <button @click="$emit('print', chosenVerse)" class="print-button">اضف
          للطباعة</button>
      </div>
    </div>
  </section>
